...
namespace ThreeByte.Serial {
    public class FramedRawSerialLink : IDisposable, INotifyPropertyChanged {
        private readonly ILog log = LogManager.GetLogger(typeof(FramedRawSerialLink));
        ...
        #region SerialLink Pass-Through Properties
        public string COMPort {
            get { return _serialLink.COMPort; }
            set { _serialLink.COMPort = value; }
        }
        ...
        /// </summary>
        public bool Enabled {
            get { return _serialLink.Enabled; }
            set {
                _serialLink.Enabled = value;
                if(!value) {
        ...
        public FramedRawSerialLink(string comPort, bool enabled = true, int baudRate = 9600) {

            _incomingBuffer = new MemoryStream(2048);
            _incomingData = new List<byte[]>();

			_serialLink = new SerialLink(comPort, enabled, baudRate, 7, System.IO.Ports.Parity.Even);
        ...
        public void Dispose() {
            ...
            _disposed = true;
            log.Info("Cleaning up serial resources");
            _serialLink.Dispose();
        }
        ...
        void _serialLink_DataReceived(object sender, EventArgs e) {
                ...
                lock(_incomingBuffer) {
                    ...
                    for(int i = 0; i < buffer.Length; i++) {
                        if(_headerPos < header.Length - 1 && buffer[i] == header[_headerPos]) {
                            ...
                            byte[] frameData = new byte[_incomingBuffer.Position];
                            Array.Copy(_incomingBuffer.GetBuffer(), frameData, frameData.Length);

                            lock(_incomingData) {
                                _incomingData.Add(frameData);
        ...
        public void SendData(byte[] message) {
            if(_disposed) {
                throw new ObjectDisposedException("Cannot send message on disposed FramedRawSerialLink");
            }
            ...
            message.CopyTo(messageBytes, header.Length);
        ...
        public byte[] GetData() {
            if(_disposed) {
                throw new ObjectDisposedException("Cannot get message from disposed FramedRawSerialLink");
            }
